...
\begin{itemize}
    \item Ausgabe als \textbf{CSV} (mit Header, auf 1 Datei zusammengeführt via \texttt{coalesce(1)})
    \item Zusätzlich Ausgabe als \textbf{Parquet} zur effizienten Weiterverarbeitung
\end{itemize}
...
\begin{verbatim}
JavaPairRDD<String, String> filesRDD = spark.sparkContext()
    .wholeTextFiles(textDir + "/*/*.txt", 8)
    .toJavaRDD()
...
Broadcast<Map<String, Set<String>>> broadcastStopwords = 
    spark.sparkContext().broadcast(stopwordsMap, ...);
...
\textbf{Analyse}: Die Skalierung zeigt deutliche Vorteile der Parallelisierung. Der Speedup ist nicht linear, da:
\begin{itemize}
    \item Shuffle-Operationen (z.B. \texttt{distinct()}, \texttt{reduceByKey}) Synchronisation erfordern
    \item Der Local Mode alle Kerne auf einem Rechner nutzt (shared memory)
    \item Overhead durch Spark-Koordination bei kleinen Datensätzen
\end{itemize}
